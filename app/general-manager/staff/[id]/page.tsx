...
import { getStaffProfileGM } from "../../actions/staff";
import ReportsSection from "@/components/reports/ReportsSection";

type Profile = Awaited<ReturnType<typeof getStaffProfileGM>>;
...
export default function GMStaffProfilePage() {
  const { id } = useParams<{ id: string }>();
  const [profile, setProfile] = useState<Profile | null>(null);

  useEffect(() => { getStaffProfileGM(id).then(setProfile); }, [id]);
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/general-manager/staff" className="text-gray-400 hover:text-gray-600 text-sm">← Staff</Link>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            {staff.branch && <p className="text-xs text-gray-500 mt-0.5">{staff.branch.name}</p>}
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="mt-1 font-medium text-gray-800">{staff.monthlySalary != null ? `$${staff.monthlySalary}` : "—"}</p>
          </div>
          <div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
        <h2 className="text-base font-semibold text-gray-800 mb-4">
          Attendance <span className="text-xs font-normal text-gray-400">last {attendances.length} records</span>
        </h2>
