...
import { getTeacherProfile } from "../../actions/teachers";
import { markReportDone } from "../../actions/reports";
import ReportsSection from "@/components/reports/ReportsSection";

type Profile = Awaited<ReturnType<typeof getTeacherProfile>>;
...
export default function TeacherProfilePage() {
  ...
  async function load() {
    setProfile(await getTeacherProfile(id));
  }
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/manager/teachers" className="text-gray-400 hover:text-gray-600 text-sm">← Teachers</Link>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            <span className="text-xs text-gray-500 uppercase tracking-wider">Payment Type</span>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
        <h2 className="text-base font-semibold text-gray-800 mb-4">
          Active Classes
          <span className="ml-2 text-xs font-normal text-gray-400">({activeClasses.length})</span>
        </h2>
          ...
          <div className="space-y-2">
            {activeClasses.map(s => (
              <div key={s.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                ...
                <Link
                  href={`/manager/classes/${s.courseClass.id}`}
      ...
      {completedClasses.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
          <h2 className="text-base font-semibold text-gray-800 mb-4">
            Completed Classes
            <span className="ml-2 text-xs font-normal text-gray-400">({completedClasses.length})</span>
          </h2>
          <div className="space-y-2">
      ...
      {proctorExams.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
          <h2 className="text-base font-semibold text-gray-800 mb-4">
            Proctored Exams
            <span className="ml-2 text-xs font-normal text-gray-400">({proctorExams.length})</span>
          </h2>
          <div className="space-y-2">
            {proctorExams.map(exam => (
              <div key={exam.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    {exam.courseClass.courseTemplate.name} · {new Date(exam.date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
                  </p>
                </div>
              </div>
            ))}
