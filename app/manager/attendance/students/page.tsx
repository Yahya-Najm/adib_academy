...
export default function StudentAttendancePage() {
  ...
  function isClassFinalized(classId: string) {
    return ctx?.finalizedClassIds.includes(classId) ?? false;
  }
  ...
  void handleRemoveClassHoliday; // used above inline

  const isHoliday = !!ctx?.holidayLabel;
  const allFinalized = ctx && ctx.classes.length > 0 && ctx.classes.every(c => isClassFinalized(c.id));
    ...
    <div className="p-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mt-0.5">Finalize each class individually to lock student records</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {ctx.classes.map(cls => {
            ...
            const finalized = isClassFinalized(cls.id);

            return (
              <div key={cls.id} className={`bg-white border rounded-xl overflow-hidden ${finalized ? "border-green-200" : "border-gray-200"}`}>
                <div className="px-5 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {finalized ? (
                      ...
                      <FinalizeClassButton courseClassId={cls.id} dateStr={dateStr} onFinalized={load} />
                    )}
                  </div>
                </div>
...
function FinalizeClassButton({ courseClassId, dateStr, onFinalized }: { courseClassId: string; dateStr: string; onFinalized: () => void }) {
  const [isPending, startTransition] = useTransition();
  ...
  function handleFinalize() {
    startTransition(async () => {
      try {
        await finalizeClassAttendance(courseClassId, dateStr);
        onFinalized();
        setConfirm(false);
      } catch (e: unknown) {
