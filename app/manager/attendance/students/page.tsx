...
export default function StudentAttendancePage() {
  ...
  function getRecord(classSectionId: string, studentId: string) {
    return ctx?.records.find(r => r.classSectionId === classSectionId && r.studentId === studentId);
  }
  ...
  function isSectionFinalized(sectionId: string) {
    return ctx?.finalizedSectionIds.includes(sectionId) ?? false;
  }

  function isSectionTeacherFinalized(sectionId: string) {
    return ctx?.teacherFinalizedSectionIds.includes(sectionId) ?? false;
  }

  function handleToggle(courseClassId: string, classSectionId: string, studentId: string, present: boolean) {
    const record = getRecord(classSectionId, studentId);
    startTransition(async () => {
      try {
        await upsertStudentAttendance({ courseClassId, classSectionId, studentId, date: dateStr, present, isLate: record?.isLate ?? false });
        load();
      } catch (e: unknown) {
  ...
  function handleLate(courseClassId: string, classSectionId: string, studentId: string, isLate: boolean) {
    const record = getRecord(classSectionId, studentId);
    if (!record) return;
    startTransition(async () => {
      try {
        await upsertStudentAttendance({ courseClassId, classSectionId, studentId, date: dateStr, present: record.present, isLate });
        load();
      } catch (e: unknown) {
        setError(e instanceof Error ? e.message : "Failed to save");
      }
  ...
  const allSections = ctx?.classes.flatMap(c => c.sections) ?? [];
  const allFinalized = allSections.length > 0 && allSections.every(s =>
    s.studentEnrollments.length === 0 || isSectionFinalized(s.id)
  );
    ...
    <div className="p-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mt-0.5">Attendance is tracked per section — finalize each section to lock records</p>
        </div>
      ...
      {allFinalized && (
        <div className="flex items-center gap-1.5 text-sm font-medium text-green-700 bg-green-50 border border-green-200 px-4 py-2 rounded-lg mb-6 w-fit">
          ...
          All class attendance finalized for this day
        </div>
      )}
              ...
              <div key={cls.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div className="px-5 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between flex-wrap gap-2">
                  <div>
                    ...
                    <span className="text-xs text-gray-500 ml-2">{cls.classTime} · {cls.sections.length} section{cls.sections.length !== 1 ? "s" : ""}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {classHolidayReason ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => handleRemoveClassHolidayByClassDate(cls.id)}
                      ...
                      holidayForm.show ? (
                        <div className="flex items-center gap-2">
                          <input type="text" placeholder="Holiday reason…" value={holidayForm.reason}
                          ...
                          <button onClick={() => handleAddClassHoliday(cls.id)} disabled={isPending || !holidayForm.reason.trim()}
                            className="text-xs font-medium text-white bg-orange-500 hover:bg-orange-600 disabled:opacity-50 px-2 py-1 rounded-md">Add</button>
                          <button onClick={() => setHolidayForms(f => ({ ...f, [cls.id]: { show: false, reason: "", error: "" } }))}
                            className="text-xs text-gray-400 hover:text-gray-600">Cancel</button>
                        </div>
                      ) : (
                        <button onClick={() => setHolidayForms(f => ({ ...f, [cls.id]: { show: true, reason: "", error: "" } }))}
                ...
                {(isHoliday || classHolidayReason) ? (
                  <div className="px-5 py-4">
                    ...
                    <div className="space-y-1">
                      {cls.sections.flatMap(s => s.studentEnrollments).map(e => (
                  ...
                  <>
                    ...
                    {cls.sections.map((section, sIdx) => {
                      const finalized = isSectionFinalized(section.id);
                      const teacherFinalized = isSectionTeacherFinalized(section.id);
                      return (
                        <div key={section.id} className={sIdx > 0 ? "border-t border-gray-100" : ""}>
                          {/* Section sub-header */}
                          <div className={`px-5 py-2 flex items-center justify-between flex-wrap gap-2 ${teacherFinalized ? "bg-purple-50" : finalized ? "bg-green-50" : "bg-gray-50/50"}`}>
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-semibold text-gray-700">
                                Section {section.sectionNumber}{section.sectionName ? ` — ${section.sectionName}` : ""}
                              </span>
                              <span className="text-xs text-gray-400">·</span>
                              <span className="text-xs text-gray-500">{section.teacher.name}</span>
                              <span className="text-xs text-gray-400">·</span>
                              <span className="text-xs text-gray-500">{section.studentEnrollments.length} students</span>
                            </div>
                            <div className="flex items-center gap-2">
                              {teacherFinalized ? (
                                <span className="text-xs text-purple-700 font-medium flex items-center gap-1 bg-purple-100 px-2 py-0.5 rounded-full">
                              ...
                              ) : section.studentEnrollments.length > 0 && (
                                <FinalizeSectionButton classSectionId={section.id} dateStr={dateStr} onFinalized={load} />
                              )}
                            </div>
                          </div>

                          {section.studentEnrollments.length === 0 ? (
                            <div className="px-5 py-3 text-xs text-gray-400">No students enrolled in this section.</div>
                          ) : (
                            <table className="w-full text-sm">
                              ...
                              <tbody className="divide-y divide-gray-50">
                                {section.studentEnrollments.map(enrollment => {
                                  const student = enrollment.student;
                                  const record = getRecord(section.id, student.id);
                                      ...
                                      <td className="px-5 py-3">
                                        {finalized ? (
                                          ...
                                          <div className="flex gap-1.5">
                                            <button onClick={() => handleToggle(cls.id, section.id, student.id, true)} disabled={isPending}
                                            ...
                                            <button onClick={() => handleToggle(cls.id, section.id, student.id, false)} disabled={isPending}
                                      ...
                                      <td className="px-5 py-3">
                                        {finalized ? (
                                          ...
                                          <input type="checkbox" checked={record?.isLate ?? false}
                                            onChange={e => handleLate(cls.id, section.id, student.id, e.target.checked)}
                                            disabled={isPending || !record}
                                            className="w-4 h-4 accent-teal-600 cursor-pointer disabled:opacity-40" />
                                        )}
                                      </td>
                                      <td className="px-5 py-3 min-w-[200px]">
                                        <ReportPanel date={dateStr} subjectType="STUDENT" subjectId={student.id}
                    ...
                    <div className="px-5 py-3 border-t border-gray-100">
                      ...
                      <ReportPanel date={dateStr} subjectType="CLASS" subjectId={cls.id}
                        reports={classReports as Parameters<typeof ReportPanel>[0]["reports"]}
                        finalized={false} onReportsChange={load} />
                    </div>
                  </>
                )}
              </div>
...
function FinalizeSectionButton({ classSectionId, dateStr, onFinalized }: { classSectionId: string; dateStr: string; onFinalized: () => void }) {
  const [isPending, startTransition] = useTransition();
  ...
  function handleFinalize() {
    startTransition(async () => {
      try {
        await finalizeClassAttendance(classSectionId, dateStr);
        onFinalized();
        setConfirm(false);
      } catch (e: unknown) {
