...
**AttendanceFinalization** — per-scope lock; `@@unique([date, branchId, finalizationType, scopeId])`. Once finalized, attendance writes for that scope+date are blocked. Reports remain addable after finalization. See **AttendanceFinalization (granular)** section below for field details.
...
The manager overview page (`/manager`) surfaces three notification panels:
- **Payment notifications** — `MonthlyPayment` rows with status `PENDING | PARTIAL | OVERDUE` due within 3 days (`getPaymentNotifications`)
- **Exam notifications** — exams within 3 days or past with unfinalized scoring (`getExamNotifications`)
- **Pending actionable reports** — `ACTIONABLE` reports with `isDone=false` (`getPendingActionableReports`)
- **Report counts** — per-date attendance report breakdown (`getReportCountsForDate`)
...
- `ReportPanel` records both `manager` and `teacher` relations — either role may author a report from the attendance UI
...
Teacher payment types: `PER_CLASS` (fixed rate), `REVENUE_PERCENTAGE` (% of student fees), or `FIXED_HOURS` (hourly rate × hours worked).
Manager/Staff payment type: `MONTHLY_SALARY`.

### Teacher dashboard

The teacher dashboard (`/teacher`) is fully implemented. Teacher scoping rules:
- A teacher only sees classes where they have a `ClassSection` assigned to them.
- Teachers can report only on `STUDENT` and `CLASS` subjects (not STAFF/TEACHER).
- Auto-generated attendance reports use the `teacherId` field (not `managerId`).
- Teachers can finalize student attendance for their class — this writes `teacherId` into `AttendanceFinalization` (same unique key as manager finalization: `date + branchId + finalizationType + scopeId`).
- Teachers can enter and finalize exam scores for exams in their classes.

Teacher pages:
- **Overview** — upcoming holidays (`OfficialHoliday`) + exam notifications (exams within 3 days or past with unfinalized scoring)
- **Classes** (`/teacher/classes`) — lists assigned classes with template name, branch, time, student count, and their own sections
...
Payments for non-student payees (staff, managers, teachers) live in `app/manager/payments/`.

**StaffPayment** fields: `userId`, `branchId`, `dueDate`, `periodStart`, `periodEnd`, `grossAmount`, `deduction`, `netAmount`, `status` (`DRAFT | PDF_GENERATED | FINALIZED`). `@@unique([userId, dueDate])`.
